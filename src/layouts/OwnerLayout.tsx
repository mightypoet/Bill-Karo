...
export default function OwnerLayout() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#F3F4F6] text-slate-900 overflow-hidden font-sans">
      {/* Mobile top-bar */}
      <div className="md:hidden flex items-center justify-between p-4 bg-emerald-600 text-white z-[60] shadow-md">
      ...
      <aside className="hidden md:flex bg-white/80 backdrop-blur-md w-64 border-r border-slate-200 z-20 h-full flex-col">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-[60px] left-0 w-full h-[calc(100vh-60px)] bg-white z-[60] flex flex-col pt-4 shadow-xl">
          ...
          <div className="p-4 border-t border-slate-200">
            <Button
              variant="ghost"
              className="w-full justify-start text-red-500 hover:text-red-700 hover:bg-red-50 font-bold px-4 py-3 text-base h-auto"
              onClick={signOut}
            >
              <LogOut className="w-6 h-6 mr-3 shrink-0" />
              Sign Out
            </Button>
          </div>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto bg-[#F3F4F6] pb-16 md:pb-0 h-full">
        <Outlet />
      </main>
    </div>
  );
